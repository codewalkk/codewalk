-- CodeGraph SQLite Schema — ported verbatim from the TS spec
-- (codewalk_kb/codegraph/src/db/schema.sql). rusqlite (bundled, fts5) statically
-- links SQLite-with-FTS5, so the `nodes_fts` virtual table below always exists —
-- this is the proof the "FTS5 cliff" is gone (docs/rust-build-plan.md §0).
-- Version 1
...
-- corresponding source-only / target-only lookups via SQLite's left-prefix scan.
